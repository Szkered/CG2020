...
void drawMesh()
{
    ...
    glLineWidth(1.0);
    for (CHodgeDecompositionMesh::MeshFaceIterator_ fiter(g_domain_mesh); !fiter.end(); ++fiter)
    {
        glBegin(GL_POLYGON);
        CHodgeDecompositionFace *pF = *fiter;
        for (CHodgeDecompositionMesh::FaceVertexIterator_ fviter(pF); !fviter.end(); ++fviter)
        {
...
void drawBoundary(int mode)
{
    ...
    glColor3f(0, 0, 1);
    glBegin(GL_LINES);
    for (CHodgeDecompositionMesh::MeshEdgeIterator_ eit(g_domain_mesh); !eit.end(); ++eit)
    {
...
{
    CPoint s(0, 0, 0);
    for (CHodgeDecompositionMesh::MeshVertexIterator_ viter(pMesh); !viter.end(); ++viter)
    {
...
void computeNormal(CHodgeDecompositionMesh *pMesh)
{
    for (CHodgeDecompositionMesh::MeshVertexIterator_ viter(pMesh); !viter.end(); ++viter)
    {
        CHodgeDecompositionVertex *v = *viter;
        CPoint n(0, 0, 0);
        for (CHodgeDecompositionMesh::VertexFaceIterator_ vfiter(v); !vfiter.end(); ++vfiter)
        {
...
bool closed_mesh(CHodgeDecompositionMesh *pM)
{
    ...
    for (M::MeshVertexIterator_ viter(pM); !viter.end(); viter++)
    {
        M::CVertex *pv = *viter;
        if (pv->boundary())
            return false;
...
void calc_holo_1_form(const std::string &input_mesh)
{
    ...
    if (!closed_mesh(pM))
    {
        ...
        for (std::vector<M *>::iterator iter = g_meshes.begin(); iter != g_meshes.end(); iter++, i++)
        {
            ...
            for (M::MeshEdgeIterator_ eiter(pM); !eiter.end(); eiter++)
            {
                M::CEdge *pe = *eiter;
                pe->duv() /= s;
            }
