...
void drawMesh() {
  ...
  for (CCutGraphMesh::MeshFaceIterator fiter(&g_mesh); !fiter.end(); ++fiter) {
    ...
    for (CCutGraphMesh::FaceVertexIterator fviter(pF); !fviter.end();
         ++fviter) {
...
void mouseClick(int button, int state, int x, int y) {
  ...
  if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
    ...
    g_arcball = CArcball(g_win_width, g_win_height, x - g_win_width / 2,
...
void mouseMove(int x, int y) {
  ...
  if (g_button == GLUT_LEFT_BUTTON) {
    rot = g_arcball.update(x - g_win_width / 2,
                           g_win_height - y - g_win_height / 2);
