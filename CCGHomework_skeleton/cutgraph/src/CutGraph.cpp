...
void MeshLib::CCutGraph::_dual_spanning_tree() {
  ...
  while (!fQueue.empty()) {
    ...
    for (CCutGraphMesh::FaceHalfedgeIterator fhiter(pF); !fhiter.end();
         ++fhiter) {
...
void MeshLib::CCutGraph::_prune() {
  ...
  for (CCutGraphMesh::MeshVertexIterator viter(m_pMesh); !viter.end();
       ++viter) {
    CCutGraphVertex *pV = *viter;
    pV->valence() = 0;

    for (CCutGraphMesh::VertexEdgeIterator veiter(pV); !veiter.end();
         ++veiter) {
  ...
  while (!vQueue.empty()) {
    ...
    for (CCutGraphMesh::VertexEdgeIterator veiter(pV); !veiter.end();
      ...
      CCutGraphVertex *pW = m_pMesh->edgeVertex1(pE) == pV
                                ? m_pMesh->edgeVertex2(pE)
                                : m_pMesh->edgeVertex1(pE);
      if (pE->sharp()) {
        // insert your code here
        break;
      }
