 ...
 */
template <typename Mesh> class COperator
{
  public:
    /*! \brief COperator constructor
     *  \param pMesh the input mesh
     */
    COperator(Mesh *pMesh)
    {
        m_pMesh = pMesh;
    };
  ...
  protected:
     ...
     */
    int _circle_circle_intersection(double x0, double y0, double r0, double x1, double y1, double r1, double *xi,
                                    double *yi, double *xi_prime, double *yi_prime);
};

// Euclidean cosine law
template <typename M> double COperator<M>::_cosine_law(double a, double b, double c)
{
...
template <typename M> void COperator<M>::_metric_2_angle()
{
...
template <typename M> void COperator<M>::_embedding_2_metric()
{
...
template <typename M> void COperator<M>::_angle_2_curvature()
{
...
// Calculate vertex curvature
template <typename M> void COperator<M>::_angle_2_Laplace()
{
...
// Calculate vertex curvature
template <typename M> void COperator<M>::_combinatorial_Laplace()
{
...
template <typename M> void COperator<M>::_parameter_mu_2_metric()
{
    for (typename M::MeshEdgeIterator eiter(m_pMesh); !eiter.end(); eiter++)
    {
        typename M::CEdge *e = *eiter;
...
template <typename M> void COperator<M>::_parameter_mu_2_angle()
{
    for (typename M::MeshFaceIterator fiter(m_pMesh); !fiter.end(); fiter++)
    {
        typename M::CFace *f = *fiter;
        ...
        for (int i = 0; i < 3; i++)
        {
            double C = (ls[(i + 1) % 3] * ls[(i + 1) % 3] + ls[(i + 2) % 3] * ls[(i + 2) % 3] - ls[i] * ls[i]) /
                       (2 * ls[(i + 1) % 3] * ls[(i + 2) % 3]);
            C = (C > 1) ? 1 : C;
            C = (C < -1) ? -1 : C;
...
template <typename M> void COperator<M>::_parameter_2_mu()
{
    for (typename M::MeshFaceIterator fiter(m_pMesh); !fiter.end(); fiter++)
    {
        typename M::CFace *f = *fiter;
...
template <typename M> void COperator<M>::_corner_angle_2_vertex_curvature()
{
...
void COperator<M>::_parameter_mu_2_angle( )
{
        for( M::MeshFaceIterator fiter( m_pMesh ); ! fiter.end(); fiter ++ )
        {
                ...
                for( int i = 0; i < 3; i ++ )
                {
                        double C = (ls[(i+1)%3] * ls[(i+1)%3] + ls[(i+2)%3] * ls[(i+2)%3] - ls[i] *
ls[i])/(2*ls[(i+1)%3] * ls[(i+2)%3]); C = ( C > 1)? 1:C; C = ( C <-1)?-1:C; hs[(i+1)%3]->angle() = acos( C );
                }
        }
};
...
template <typename M> void COperator<M>::_metric_2_diagonal_ratio()
{
    for (typename M::MeshEdgeIterator eiter(m_pMesh); !eiter.end(); eiter++)
    {
...
template <typename M> std::complex<double> COperator<M>::_diagonal_ratio(M *pM, typename M::CEdge *pE)
{
...
template <typename M> void COperator<M>::_embed_two_faces(M *pM, typename M::CEdge *pE)
{
    typename M::CHalfEdge *pH = pM->edgeHalfedge(pE, 0);
    typename M::CHalfEdge *pN = pM->faceNextCcwHalfEdge(pH);
    typename M::CHalfEdge *pD = pM->edgeHalfedge(pE, 1);
...
template <typename M> void COperator<M>::_embed_one_face(M *pM, typename M::CEdge *pE)
{
    typename M::CHalfEdge *pH = pM->edgeHalfedge(pE, 0);
    typename M::CHalfEdge *pN = pM->faceNextCcwHalfEdge(pH);
...
template <typename M>
int COperator<M>::_circle_circle_intersection(double x0, double y0, double r0, double x1, double y1, double r1,
...
template <typename M> void COperator<M>::_calculate_face_vertex_normal()
{
...
template <typename M> void COperator<M>::_calculate_face_vertex_area()
{
...
template <typename M> void COperator<M>::_normalize()
{
    CPoint s(0, 0, 0);
    for (typename M::MeshVertexIterator viter(m_pMesh); !viter.end(); ++viter)
    {
...
template <typename M> void COperator<M>::_uv_2_pos()
{
    for (typename M::MeshVertexIterator viter(m_pMesh); !viter.end(); ++viter)
    {
        typename M::CVertex *v = *viter;
...
template <typename M> void COperator<M>::_set_vertex_default_color()
{
    bool has_rgb = false;
    for (typename M::MeshVertexIterator viter(m_pMesh); !viter.end(); viter++)
    {
