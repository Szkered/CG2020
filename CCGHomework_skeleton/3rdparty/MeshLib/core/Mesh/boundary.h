...
template <typename CVertex, typename CEdge, typename CFace, typename CHalfEdge> class CLoop
{
...
template <typename CVertex, typename CEdge, typename CFace, typename CHalfEdge> class CBoundary
{
...
CBoundary<CVertex, CEdge, CFace, CHalfEdge>::CBoundary(CBaseMesh<CVertex, CEdge, CFace, CHalfEdge> *pMesh)
{
    ...
    while (!boundary_hes.empty())
    {
        ...
        for (typename std::list<CHalfEdge *>::iterator hiter = pL->halfedges().begin(); hiter != pL->halfedges().end();
             hiter++)
        {
            CHalfEdge *he = *hiter;
