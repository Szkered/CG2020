 ...
 */
template <typename V, typename E, typename F, typename H> class CDynamicMesh : public CBaseMesh<V, E, F, H>
{
  public:
...
template <typename V, typename E, typename F, typename H> CDynamicMesh<V, E, F, H>::~CDynamicMesh()
{
}
        ...
        else if (token == "Face")
        {
            ...
            while (stokenizer.nextToken())
            {
                ...
                if (strutil::startsWith(token, "+"))
                    he_ind = 0;
                else if (strutil::startsWith(token, "-"))
                    he_ind = 1;
                else
                    break;
