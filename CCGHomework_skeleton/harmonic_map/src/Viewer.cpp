...
void drawMesh()
{
    ...
    glLineWidth(1.0);
    for (CHarmonicMapMesh::MeshFaceIterator_ fiter(&g_mesh); !fiter.end(); ++fiter)
    {
        glBegin(GL_POLYGON);
        CHarmonicMapFace *pF = *fiter;
        for (CHarmonicMapMesh::FaceVertexIterator_ fviter(pF); !fviter.end(); ++fviter)
        {
...
{
    CPoint s(0, 0, 0);
    for (CHarmonicMapMesh::MeshVertexIterator_ viter(pMesh); !viter.end(); ++viter)
    {
...
void computeNormal(CHarmonicMapMesh *pMesh)
{
    for (CHarmonicMapMesh::MeshVertexIterator_ viter(pMesh); !viter.end(); ++viter)
    {
        CHarmonicMapVertex *v = *viter;
        CPoint n(0, 0, 0);
        for (CHarmonicMapMesh::VertexFaceIterator_ vfiter(v); !vfiter.end(); ++vfiter)
        {
