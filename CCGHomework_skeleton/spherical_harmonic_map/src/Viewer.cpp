...
void drawMesh()
{
    ...
    for (CSHMMesh::MeshFaceIterator_ fiter(&g_mesh); !fiter.end(); ++fiter)
    {
        glBegin(GL_POLYGON);
        CSHMMesh::CFace *pF = *fiter;
        for (CSHMMesh::FaceVertexIterator_ fviter(pF); !fviter.end(); ++fviter)
        {
...
{
    CPoint s(0, 0, 0);
    for (CSHMMesh::MeshVertexIterator_ viter(pMesh); !viter.end(); ++viter)
    {
...
{
    for (CSHMMesh::MeshFaceIterator_ fiter(pMesh); !fiter.end(); ++fiter)
    {
        CSHMMesh::CFace *pF = *fiter;
        CPoint p[3];
